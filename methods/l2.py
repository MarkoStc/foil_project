...
def run_naive_l2(
    ...
    """
    Naive L2 continual learning:
      L = CE + (lambda/2) * ||theta - theta_old||^2
    where theta_old is the parameter snapshot after finishing task (t-1).
    """
    if device is None:
    ...
    theta_old: Optional[List[torch.Tensor]] = None
    ...
    for task_id in range(num_tasks):
        ...
        for _ in range(cfg.epochs_per_task):
            ...
            for x, y in train_loaders[task_id]:
                ...
                if theta_old is not None and cfg.lambda_l2 > 0.0:
                    l2 = 0.0
                    for p, p_old in zip(model.parameters(), theta_old):
                        l2 = l2 + torch.sum((p - p_old) ** 2)
                    loss = loss + 0.5 * cfg.lambda_l2 * l2
        ...
        theta_old = snapshot_params(model, device=device)
